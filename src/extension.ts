...
export function activate(context: vscode.ExtensionContext) {
  ...
  function matchCursor(
    ...
    if (position >= match.index && position <= match.index + match[0].length) {
      for (let i = 0; i < match[0].length; i++) {
        const offset = match.index + i;
        ranges.push(
          new vscode.Range(lineNumber, offset, lineNumber, offset + 1)
        );
      }
    }
    ...
    selections.forEach((selection) => {
      ...
      if (charPositions) {
        ...
        positions.set(
          ...
          new Set<number>([selection.start.character])
        );
      }
      ...
      if (charPositions) {
        ...
        positions.set(
          selection.end.line,
          new Set<number>([selection.end.character])
        );
      }
    });
  ...
  function readConfiguration(): { mode: string; regex: RegExp } {
    const ligatureConfiguration = vscode.workspace
      .getConfiguration()
      .get("disableLigatures") as any;
